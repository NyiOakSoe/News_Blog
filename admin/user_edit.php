    ...
    if(empty($_POST['name']) || empty($_POST['email'])) {
        ...
        if($user_result){
            echo "<script>alert('Email already')</script>";
             ...
             if($password !=null){
                $stmt2=$pdo->prepare("UPDATE users SET name='$name',email='$email',role='$role' WHERE id=$id");
             }else{
                    ...
                    <center><h4>Account Edit</h4></center>
                        ...
                        <input type="hidden" name="_token" value="<?php echo $_SESSION['_token'];?>">
                            ...
                            <div class="form-group">
                                ...
                                <input type="email" name="email" class="form-control" id=""  value="<?php echo escape($result[0]['email']);?>">
                                <p style="color:red"><?php echo !empty($emailError)?$emailError:'';?></p>
                            </div>
                            <div class="form-group">
                                <label for="">Password</label><br>
                                <span style="font-size:15px; color:green">*The user already has a password</span>
